        ...
        stage('Test and deploy the application in preproduction') {
             ...
             stages {
               ...
               stage("Verify ansible playbook syntax") {
                   steps {
                       sh 'ansible-lint -x 306 install-bulletin-board.yml'
                       sh 'echo "${GIT_BRANCH}"'
                   }
               }
               ...
               stage("Build docker images on build host") {
                   ...
                   steps {
                       sh 'ansible-playbook  -i hosts --vault-password-file vault.key --private-key id_rsa --tags "build" --limit build install-bulletin-board.yml'
                   }
               }
               ...
               stage("Deploy application in preproduction") {
                  when {
                   ...
                   steps {
                       sh 'ansible-playbook  -i hosts --vault-password-file vault.key --private-key id_rsa --tags "preprod" --limit preprod install-bulletin-board.yml'
                   }
               }
        ...
        stage('Test and deploy the application in production') {
            ...
            stages {
               ...
               stage("Deploy application in production") {
                   ...
                   steps {
                       sh 'ansible-playbook  -i hosts --vault-password-file vault.key --private-key id_rsa --tags "prod" --limit prod install-bulletin-board.yml'
                   }
               }
